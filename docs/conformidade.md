...
Este relatório analisa a conformidade do sistema "Gerador de PR e Release Notes" com os requisitos funcionais e não funcionais especificados no documento de requisitos. A análise foi realizada através de inspeção direta do código-fonte e dos artefatos do projeto.
...
**Evidências:**
- [cli.py:12](src/gerador_cli/cli.py#L12): Argumento `repo` aceita caminho para repositório Git local (default: `.`)
- [workflows/sync.py:122-124](src/gerador_cli/workflows/sync.py#L122-L124): Validação do diretório do repositório
- [adapters/subprocess.py](src/gerador_cli/adapters/subprocess.py): Função `run_git()` executa comandos git no repositório
...
**Evidências:**
- [cli.py:13-15](src/gerador_cli/cli.py#L13-L15): Parâmetros `--range`, `--since`, `--until` permitem configurar intervalo
- [data_collection.py:85-127](src/gerador_cli/domain/services/data_collection.py#L85-L127): Função `get_commits()` extrai commits com suporte a:
...
**Evidências:**
- [models.py:16-37](src/gerador_cli/domain/models.py#L16-L37): Dataclass `Commit` contém todos os metadados requeridos:
  - `sha` (hash)
  ...
  - `diff` (diff completo)
- [data_collection.py:34-82](src/gerador_cli/domain/services/data_collection.py#L34-L82): Função `parse_git_log()` extrai todos esses metadados
- [data_collection.py:112-126](src/gerador_cli/domain/services/data_collection.py#L112-L126): Busca assíncrona de body e diff em paralelo
...
**Evidências:**
- [domain_definition.py:24-50](src/gerador_cli/adapters/domain_definition.py#L24-L50): Constante `TEXT_EXTS` define extensões documentais configuráveis:
  ```python
  TEXT_EXTS = {".md", ".txt", ".rst", ".py", ".js", ".ts", ".tsx", ".jsx",
               ".java", ".cs", ".go", ".rb", ".php", ".sh", ".yml", ".yaml",
               ".json", ".toml", ".ini", ".cfg", ".conf", ".xml", ".sql", ...}
  ```
- [domain_definition.py:151-152](src/gerador_cli/adapters/domain_definition.py#L151-L152): Priorização de READMEs e arquivos de configuração
- [domain_definition.py:178-187](src/gerador_cli/adapters/domain_definition.py#L178-L187): Função `iter_repo_files()` varre repositório respeitando regras de ignore
...
**Evidências:**
- [domain_profile.py:51-116](src/gerador_cli/adapters/domain_profile.py#L51-L116): Função `generate_domain_profile()` implementa o fluxo completo:
  - Concatena conteúdo de arquivos documentais
  - Envia ao modelo de linguagem
  - Solicita preenchimento de template estruturado
- [schemas.py:190-197](src/gerador_cli/domain/schemas.py#L190-L197): Schema `ProjectProfile` contém campos estruturados:
...
**Evidências:**
- [domain_profile.py:119-125](src/gerador_cli/adapters/domain_profile.py#L119-L125): Função `save_domain_profile()` persiste em JSON
- [domain_profile.py:128-131](src/gerador_cli/adapters/domain_profile.py#L128-L131): Função `load_domain_profile()` carrega perfil existente
- [workflows/sync.py:82-118](src/gerador_cli/workflows/sync.py#L82-L118): Lógica de cache - verifica existência antes de recomputar
...
**Evidências:**
- [aggregation.py:49-60](src/gerador_cli/domain/services/aggregation.py#L49-L60): Função `classify_commit()` classifica usando padrões regex configuráveis
...
- [aggregation.py:16-46](src/gerador_cli/domain/services/aggregation.py#L16-L46): Suporte a padrões JS-style (`/.../flags`) e regex Python
...
**Evidências:**
- [aggregation.py:49-60](src/gerador_cli/domain/services/aggregation.py#L49-L60): Retorna `("other", False)` quando nenhum padrão é encontrado
- [workflows/sync.py:75-79](src/gerador_cli/workflows/sync.py#L75-L79): Função `_warn_on_non_conventional()` emite alerta ao usuário:
  ```python
...
**Evidências:**
- [aggregation.py:82-97](src/gerador_cli/domain/services/aggregation.py#L82-L97): Função `group_commits_by_type()` agrupa commits por tipo
...
**Evidências:**
- [aggregation.py:63-79](src/gerador_cli/domain/services/aggregation.py#L63-L79): Função `compute_importance()` calcula score usando:
...
**Evidências:**
- [aggregation.py:74-78](src/gerador_cli/domain/services/aggregation.py#L74-L78): Mapeamento para faixas qualitativas
- [config.default.json:21-26](config.default.json#L21-L26): Faixas configuráveis:
  - low (min: 0.0)
  - medium (min: 3.0)
  - high (min: 6.0)
  - critical (min: 9.0)
- [models.py:32](src/gerador_cli/domain/models.py#L32): Campo `importance_band` no modelo Commit
...
**Evidências:**
- [aggregation.py:87-88](src/gerador_cli/domain/services/aggregation.py#L87-L88): Ordenação por `importance_score` (decrescente) dentro de cada grupo:
  ```python
...
- [domain_profile.txt:13-18](src/gerador_cli/prompts/domain_profile.txt#L13-L18): Instruções explícitas para usar apenas informações presentes no repositório:
  ```
  RULES:
  ...
  ```
- [data_collection.py:130-206](src/gerador_cli/domain/services/data_collection.py#L130-L206): Extração de "diff anchors" (keywords, artifacts) para ancorar resumos
- [aggregation.py:104-138](src/gerador_cli/domain/services/aggregation.py#L104-L138): Formatação de anchors no prompt para contextualização
...
**Evidências:**
- [models.py:29-33](src/gerador_cli/domain/models.py#L29-L33): Campos armazenados no modelo Commit:
  - `change_type`, `is_conventional`, `importance_score`, `importance_band`, `summary`
- [export.py:58-64](src/gerador_cli/domain/services/export.py#L58-L64): Função `export_commits()` persiste dados em JSON
- [workflows/sync.py:164](src/gerador_cli/workflows/sync.py#L164): Exportação de commits para `utils/commit.json`
...
**Evidências:**
- [templates/pr.md](src/gerador_cli/templates/pr.md): Template para Pull Request com campos:
  - title, summary, changes_by_type, risks, testing, alerts
- [templates/release.md](src/gerador_cli/templates/release.md): Template para Release Notes com campos:
...
- [composition.py:129-135](src/gerador_cli/domain/services/composition.py#L129-L135): Função `render_template()` para renderização
...
**Evidências:**
- [composition.py:88-126](src/gerador_cli/domain/services/composition.py#L88-L126): Função `build_release_fields()` incorpora `domain_context` no prompt
- [release_fields.txt:9-10](src/gerador_cli/prompts/release_fields.txt#L9-L10): Template inclui contexto de domínio:
  ```
  Domain context (for understanding the project):
  {{domain_context}}
  ```
- [workflows/sync.py:216](src/gerador_cli/workflows/sync.py#L216): Conversão do perfil de domínio para JSON no prompt
...
**Evidências:**
- [composition.py:53-85](src/gerador_cli/domain/services/composition.py#L53-L85): `build_pr_fields()` gera campos abstratos via LLM:
  - title, summary, risks, testing
- [composition.py:88-126](src/gerador_cli/domain/services/composition.py#L88-L126): `build_release_fields()` gera campos abstratos via LLM:
  - executive_summary, highlights, migration_notes, known_issues, internal_notes
- [schemas.py:42-87](src/gerador_cli/domain/schemas.py#L42-L87): Schemas Pydantic com validação para garantir estrutura
...
**Evidências:**
- [export.py:75-92](src/gerador_cli/domain/services/export.py#L75-L92): Funções `export_release()` e `export_pr()` salvam em Markdown
...
**Evidências:**
- [aggregation.py:300-319](src/gerador_cli/domain/services/aggregation.py#L300-L319): Função `build_convention_report()` gera relatório com:
  - Total de commits
  - Commits convencionais vs. outros
  - Exemplos de boas e más práticas
- [workflows/sync.py:158-159](src/gerador_cli/workflows/sync.py#L158-L159): Exportação para `utils/conventions.md`
...
- [cli.py:10-23](src/gerador_cli/cli.py#L10-L23): Overrides via CLI:
  - `--config`, `--range`, `--since`, `--until`, `--generate`, `--version`, `--output-dir`, `--model`, `--no-llm`
- [config.py:33-121](src/gerador_cli/config.py#L33-L121): Validação robusta de configurações
...
- [http.py:15-45](src/gerador_cli/adapters/http.py#L15-L45): Cliente Ollama executa localmente
...
- [cli.py:21](src/gerador_cli/cli.py#L21): Opção `--model` permite override
...
**Evidências:**
- [data_collection.py:119-126](src/gerador_cli/domain/services/data_collection.py#L119-L126): Processamento paralelo de commits com `ThreadPoolExecutor`
- [workflows/sync.py:140-150](src/gerador_cli/workflows/sync.py#L140-L150): Busca paralela de commits e perfil de domínio
- [llm_structured.py:21-27](src/gerador_cli/adapters/llm_structured.py#L21-L27): Timeout configurável para chamadas LLM
...
**Evidências:**
- [llm_structured.py:79-125](src/gerador_cli/adapters/llm_structured.py#L79-L125): Retry com feedback de erro (até 3 tentativas)
- [aggregation.py:281-295](src/gerador_cli/domain/services/aggregation.py#L281-L295): Fallback para subjects quando LLM falha:
  ```python
  except Exception as exc:
      print(f"WARNING: Falha ao resumir grupo {type_label}: {exc}. Usando assuntos como fallback.")
  ```
- [workflows/sync.py:186-187](src/gerador_cli/workflows/sync.py#L186-L187): Mensagens de erro claras ao usuário
- [domain_profile.py:96-100](src/gerador_cli/adapters/domain_profile.py#L96-L100): Try/catch para carregar perfil existente
- [errors.py:4-6](src/gerador_cli/domain/errors.py#L4-L6): Exceções de domínio específicas
...
**Evidências:**
- [cli.py:11](src/gerador_cli/cli.py#L11): Descrição clara no parser: "Generate PR descriptions and release notes from a Git repo"
- [cli.py:12-22](src/gerador_cli/cli.py#L12-L22): Help text para cada argumento
- [workflows/sync.py:79](src/gerador_cli/workflows/sync.py#L79): Mensagens de alerta em português para usuários brasileiros
- [config.py:116-121](src/gerador_cli/config.py#L116-L121): Mensagens de erro descritivas para configuração inválida
...
- [aggregation.py:100-101](src/gerador_cli/domain/services/aggregation.py#L100-L101): Função `build_language_hint()` gera hint de idioma para prompts
- [domain_definition.py:54-143](src/gerador_cli/adapters/domain_definition.py#L54-L143): Stopwords em português e inglês
